...
Frozen at 0.0.41, the last Stainless release. `whop_sdk` is generated by Fern from
`sdks/fern` in the whop monorepo and released by `.github/workflows/publish-main.yml`,
which does not write this file — later releases are this repository's tags. Kept through
`.fernignore` so the history below survives regeneration.
