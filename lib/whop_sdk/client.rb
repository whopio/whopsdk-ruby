...
module WhopSDK
  class Client < WhopSDK::Internal::Transport::BaseClient
    ...
    #
    # Ads billing combines eligible spend across the account's campaigns. A failed
    # payment blocks delivery with `delivery_status: payment_failed` while preserving
    # the configured active/paused `status`. Fix the account's payment method and
    # [retry its ads payment](/api-reference/beta/accounts/retry-failed-ads-payments)
    # once for the account. The retry is asynchronous: acceptance does not confirm
    # payment. Successful settlement clears the block; active campaigns can resume if
    # otherwise eligible, while paused campaigns stay paused. See
