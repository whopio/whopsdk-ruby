...
module Whop_sdk
  module PartnerReferralRequests
    class Client
      ...
      # holds the owner role. Filters narrow that combined view. Use a Whop login session or an account API key with
      # `partner:referral_request:read`. The key must have been created by the account's current owner. Account API keys
      # return their owner's sent requests and incoming requests for the key's account.
      #
      ...
      # returns 200; a new request returns 201. Use a Whop login session or an account API key with
      # `partner:referral_request:create`. The key must have been created by the account's current owner. Account API
      # keys submit requests as their account owner, who must be enrolled, verified, and not suspended.
      #
      # @param request_options [Hash]
      ...
      # Retrieves a request visible to its eligible sender or a current owner of the receiving account. Use a Whop login
      # session or an account API key with `partner:referral_request:read`. The key must have been created by the
      # account's current owner. Account API keys can retrieve their owner's sent requests and incoming requests for the
      # key's account.
      #
      # @param request_options [Hash]
      # @param params [Hash]
