...
**dispute_fighter_enabled:** `Internal::Types::Boolean` — Whether Whop assembles and files dispute evidence for this account. Enabling it opts into the success fee charged on disputes it wins. Requires payment:dispute. Omit to preserve the existing setting or creation default.
...
**orchestration_enabled:** `Internal::Types::Boolean` — Whether payment orchestration is enabled for this account. Requires payout:account:update. Omit to preserve the existing setting or creation default.
...
**send_customer_emails:** `Internal::Types::Boolean` — Whether Whop sends transactional emails to customers on behalf of the connected account.
...
<details><summary><code>client.accounts.<a href="/lib/whop_sdk/accounts/client.rb">delete</a>(id:) -> Whop_sdk::Accounts::Types::DeleteAccountsResponse</code></summary>
<dl>
<dd>
...
**eula:** `Whop_sdk::Accounts::Types::UpdateAccountsRequestEula` — The account's end-user license agreement document. PDF only. Pass a JSON object containing an `id` from [Create File](/api-reference/files/create-file), or `null` to remove it.
...
**privacy_policy:** `Whop_sdk::Accounts::Types::UpdateAccountsRequestPrivacyPolicy` — The account's privacy policy document. PDF only. Pass a JSON object containing an `id` from [Create File](/api-reference/files/create-file), or `null` to remove it.
...
**return_policy:** `Whop_sdk::Accounts::Types::UpdateAccountsRequestReturnPolicy` — The account's return and refund policy document. Attached to new disputes as the refund policy evidence, with the terms of service as the fallback. PDF only. Pass a JSON object containing an `id` from [Create File](/api-reference/files/create-file), or `null` to remove it.
...
**shipping_policy:** `Whop_sdk::Accounts::Types::UpdateAccountsRequestShippingPolicy` — The account's shipping policy document. Sent with physical-goods dispute evidence. PDF only. Pass a JSON object containing an `id` from [Create File](/api-reference/files/create-file), or `null` to remove it.
...
**budget_amount:** `Integer` — The campaign's budget in USD, which is what it is stored and billed in. Required when budget_optimization is `ad_campaign` (or send budget_amount_local instead); omit when each ad group sets its own budget.
...
**budget_amount_local:** `Integer` — The campaign's budget stated in the account's ads reporting currency (`budget_currency` on the response) instead of USD. Converted to USD at the current exchange rate and stored as budget_amount. Provide this or budget_amount, not both.
...
**budget_amount:** `Integer` — The campaign budget in USD, which is what it is stored and billed in. Interpreted as daily or lifetime per the campaign's budget type, including a budget_type sent in the same request.
...
**budget_amount:** `Integer` — This ad group's budget in USD, which is what it is stored and billed in. Omit when the budget is set on the campaign instead.
...
**descriptions:** `Internal::Types::Array[Whop_sdk::Ads::Types::CreateAdsRequestDescriptionsItem]` — The description shown on the ad. Entries without a language are the ad's own copy; add one entry per other language on a Meta ad with `translations`.
...
**descriptions:** `Internal::Types::Array[Whop_sdk::Ads::Types::UpdateAdsRequestDescriptionsItem]` — The description shown on the ad. Entries without a language are the ad's own copy; add one entry per other language on a Meta ad with `translations`.
...
**translations:** `Whop_sdk::Ads::Types::UpdateAdsRequestTranslations` — Shows a Meta ad in other languages. Each viewer sees the version for their language; everyone else sees the ad's own copy. Tag every copy and creatives entry with its language: the ad's own with `source_language`, and give every other language a `primary_texts` and `headlines` entry (a `descriptions` entry and a `creatives` entry are optional), or list it in `automatic_languages`. Needs a website destination, one image or video, and exactly one primary text and headline of the ad's own (and at most one description), with no Dynamic Creative or crops. Replaced as a whole when sent, so send `automatic_languages` with `source_language`. null turns translations off and deletes their media. Meta-only.
...
**agent_identifier:** `Whop_sdk::Types::AiChatAgentIdentifiers` 
...
**agent_identifier:** `Whop_sdk::Types::AiChatAgentIdentifiers` — The AI agent that handles the chat. Defaults to `support`.
...
**audience_type:** `Whop_sdk::Audiences::Types::ListAudiencesRequestAudienceType` — Filter by custom or lookalike audiences.
...
**source_type:** `Whop_sdk::Audiences::Types::ListAudiencesRequestSourceType` — Filter by uploaded customer lists, Whop People filters, or social engagement.
...
**audience_type:** `Whop_sdk::Audiences::Types::CreateAudiencesRequestAudienceType` — Audience type. Defaults to `custom`.
...
**column_mapping:** `Whop_sdk::Audiences::Types::CreateAudiencesRequestColumnMapping` — CSV audiences only. Maps supported identity fields to CSV column headers. Map at least one of `email` or `phone`.
...
**engagement:** `Whop_sdk::Audiences::Types::CreateAudiencesRequestEngagement` — Rules for membership based on social engagement. Requires a connected social account with advertising access.
...
**source_type:** `Whop_sdk::Audiences::Types::CreateAudiencesRequestSourceType` — Custom audience source. Inferred from `engagement`, then `filters`, otherwise defaults to `csv_upload`. Supply only the fields for the selected source.
...
**billing:** `Whop_sdk::Cards::Types::UpdateCardsRequestBilling` — The billing address. On an issued card this replaces the card's billing address and region is also required. On an invited card, sending it as the invited user completes onboarding and starts card provisioning.
...
**cardholder:** `Whop_sdk::Cards::Types::UpdateCardsRequestCardholder` — Details for the invited cardholder, accepted only while completing onboarding on an invited card. The legal name comes from an approved identity verification when the invited user has one, and from these fields when they do not.
...
**amount:** `Integer` — Amount to prefill on hosted deposit page. Crypto deposits require a $10 minimum.
...
**account_id:** `String` — Account ID, prefixed `biz_`. Defaults to the API key's own account; omit for personal onboarding.
...
**resource_id:** `String` — Optional prefixed resource ID. Returns activity associated with that resource.
...
**available_after:** `String` — Only include balance credits and debits available on or after this `YYYY-MM-DD` date (UTC), distinct from posted_at. Requires currency.
...
**available_before:** `String` — Only include balance credits and debits available on or before this `YYYY-MM-DD` date (UTC). Set equal to available_after for a single day. Requires currency.
...
**payment_method_id:** `String` — The ID of a payment method the customer has saved with your account. Future renewals charge it, and an open past-due payment is retried on it right away. Requires the `member:payment_methods:manage` permission.
...
<details><summary><code>client.memberships.<a href="/lib/whop_sdk/memberships/client.rb">resync_access</a>(id:) -> Whop_sdk::Types::Membership</code></summary>
<dl>
<dd>
...
<details><summary><code>client.partner_referral_requests.<a href="/lib/whop_sdk/partner_referral_requests/client.rb">list</a>() -> Whop_sdk::PartnerReferralRequests::Types::ListPartnerReferralRequestsResponse</code></summary>
<dl>
<dd>
...
**account_id:** `String` — Only requests for this business ID, prefixed `biz_`.
...
**target_user_id:** `String` — Only requests received by this user ID, prefixed `user_`.
...
**partner_id:** `String` — Only requests sent by this partner's user ID, prefixed `user_`.
...
**status:** `Whop_sdk::PartnerReferralRequests::Types::ListPartnerReferralRequestsRequestStatus` — Only requests with this approval status.
...
**request_type:** `Whop_sdk::PartnerReferralRequests::Types::ListPartnerReferralRequestsRequestRequestType` — Only requests initiated in this way.
...
**order:** `Whop_sdk::PartnerReferralRequests::Types::ListPartnerReferralRequestsRequestOrder` — Field used to sort requests.
...
**direction:** `Whop_sdk::PartnerReferralRequests::Types::ListPartnerReferralRequestsRequestDirection` — Sort direction.
...
**request_options:** `Whop_sdk::PartnerReferralRequests::RequestOptions` 
...
<details><summary><code>client.partner_referral_requests.<a href="/lib/whop_sdk/partner_referral_requests/client.rb">create</a>(request) -> Whop_sdk::Types::PartnerReferralRequest</code></summary>
<dl>
<dd>
...
**request:** `Whop_sdk::PartnerReferralRequests::Types::CreatePartnerReferralRequestsRequestBody` 
...
<details><summary><code>client.partner_referral_requests.<a href="/lib/whop_sdk/partner_referral_requests/client.rb">retrieve</a>(id:) -> Whop_sdk::Types::PartnerReferralRequest</code></summary>
<dl>
<dd>
...
<details><summary><code>client.partner_referral_requests.<a href="/lib/whop_sdk/partner_referral_requests/client.rb">accept</a>(id:) -> Whop_sdk::Types::PartnerReferralRequest</code></summary>
<dl>
<dd>
...
<details><summary><code>client.partner_referral_requests.<a href="/lib/whop_sdk/partner_referral_requests/client.rb">cancel</a>(id:) -> Whop_sdk::Types::PartnerReferralRequest</code></summary>
<dl>
<dd>
...
<details><summary><code>client.partner_referral_requests.<a href="/lib/whop_sdk/partner_referral_requests/client.rb">decline</a>(id:) -> Whop_sdk::Types::PartnerReferralRequest</code></summary>
<dl>
<dd>
...
**account_id:** `String` — Account to register the domain for (`biz_` tag). Required when authenticating as a user; an account API key supplies its own account.
...
**action:** `Whop_sdk::PaymentRules::Types::ReplacePaymentRulesRequestAction` — What this account's rule requests when every condition matches. One applicable account-rule action wins, in this order: `allow`, `block`, `review`, `enforce_3ds`. An `allow` overrides this account's other rules, never Whop's own fraud controls. A `review` requests authorization without capture for an eligible on-session card payment through Whop Payments. Automatic capture is scheduled for 48 hours after authorization; capture or void the payment before then to decide sooner. Capture may complete later or fail. Review is skipped for unsupported methods, off-session payments, and payments already configured for manual capture. An `enforce_3ds` is skipped when the account rule cannot apply a challenge. Other 3DS requirements still apply.
...
**capture:** `Internal::Types::Boolean` — Whether to capture a card payment immediately. Defaults to true. Pass false to place an authorization hold that must be captured in full within five days via the capture endpoint, or automatically after `auto_capture_after_minutes`.
...
<details><summary><code>client.payments.<a href="/lib/whop_sdk/payments/client.rb">resume</a>(payment_id:) -> Whop_sdk::Types::PaymentStatus</code></summary>
<dl>
<dd>
...
**event_within_days:** `Integer` — Match activity within a rolling number of days. Cannot be combined with event_from/event_to.
...
**from:** `String` — Inclusive activity-window start. Alias for event_from.
...
**to:** `String` — Inclusive activity-window end. Alias for event_to.
...
**event_from:** `String` — The inclusive start of the matching activity window.
...
**event_to:** `String` — The inclusive end of the matching activity window, for both stats drilldowns and saved audiences.
...
**ltv_gt:** `Integer` — Select people whose lifetime ltv is greater than this value. LTV and AOV are in USD.
...
**ltv_gte:** `Integer` — Select people whose lifetime ltv is at least this value. LTV and AOV are in USD.
...
**ltv_lt:** `Integer` — Select people whose lifetime ltv is less than this value. LTV and AOV are in USD.
...
**ltv_lte:** `Integer` — Select people whose lifetime ltv is at most this value. LTV and AOV are in USD.
...
**aov_gt:** `Integer` — Select people whose lifetime aov is greater than this value. LTV and AOV are in USD.
...
**aov_gte:** `Integer` — Select people whose lifetime aov is at least this value. LTV and AOV are in USD.
...
**aov_lt:** `Integer` — Select people whose lifetime aov is less than this value. LTV and AOV are in USD.
...
**aov_lte:** `Integer` — Select people whose lifetime aov is at most this value. LTV and AOV are in USD.
...
**purchase_count_gt:** `Integer` — Select people whose lifetime purchase_count is greater than this value. LTV and AOV are in USD.
...
**purchase_count_gte:** `Integer` — Select people whose lifetime purchase_count is at least this value. LTV and AOV are in USD.
...
**purchase_count_lt:** `Integer` — Select people whose lifetime purchase_count is less than this value. LTV and AOV are in USD.
...
**purchase_count_lte:** `Integer` — Select people whose lifetime purchase_count is at most this value. LTV and AOV are in USD.
...
**event_count_gt:** `Integer` — Select people whose lifetime event_count is greater than this value. LTV and AOV are in USD.
...
**event_count_gte:** `Integer` — Select people whose lifetime event_count is at least this value. LTV and AOV are in USD.
...
**event_count_lt:** `Integer` — Select people whose lifetime event_count is less than this value. LTV and AOV are in USD.
...
**event_count_lte:** `Integer` — Select people whose lifetime event_count is at most this value. LTV and AOV are in USD.
...
**contactable:** `Internal::Types::Boolean` — People metric only: contactable equals this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**has_purchased:** `Internal::Types::Boolean` — People metric only: has_purchased equals this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**first_seen_after:** `String` — People metric only: first_seen_at greater than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**first_seen_before:** `String` — People metric only: first_seen_at less than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**last_seen_after:** `String` — People metric only: last_seen_at greater than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**last_seen_before:** `String` — People metric only: last_seen_at less than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**first_seen_within_days:** `Integer` — People metric only: first_seen_at within this many days of now. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**last_seen_within_days:** `Integer` — People metric only: last_seen_at within this many days of now. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**known:** `Internal::Types::Boolean` — People metric only: known equals this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**has_email:** `Internal::Types::Boolean` — People metric only: has_email equals this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**has_phone:** `Internal::Types::Boolean` — People metric only: has_phone equals this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**ltv_gt:** `Integer` — People metric only: ltv greater than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**ltv_gte:** `Integer` — People metric only: ltv greater than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**ltv_lt:** `Integer` — People metric only: ltv less than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**ltv_lte:** `Integer` — People metric only: ltv less than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**aov_gt:** `Integer` — People metric only: aov greater than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**aov_gte:** `Integer` — People metric only: aov greater than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**aov_lt:** `Integer` — People metric only: aov less than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**aov_lte:** `Integer` — People metric only: aov less than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**purchase_count_gt:** `Integer` — People metric only: purchase_count greater than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**purchase_count_gte:** `Integer` — People metric only: purchase_count greater than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**purchase_count_lt:** `Integer` — People metric only: purchase_count less than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**purchase_count_lte:** `Integer` — People metric only: purchase_count less than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**event_count_gt:** `Integer` — People metric only: event_count greater than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**event_count_gte:** `Integer` — People metric only: event_count greater than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**event_count_lt:** `Integer` — People metric only: event_count less than this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**event_count_lte:** `Integer` — People metric only: event_count less than or equal this value. Applies to the current person profile for every time bucket. LTV and AOV are in USD. Not accepted by the Events metric.
...
**include_balance:** `Internal::Types::Boolean` — Compute live wallet and owned-account balances (default true). Set false for identity-only reads. Ignored for callers without balance-read scope.
...
**include_balance:** `Internal::Types::Boolean` — Compute live wallet and owned-account balances on the self view (default true). Set false for identity-only reads. Ignored when the id is not `me` or the caller lacks balance-read scope.
...
**covered_payout_fees:** `Whop_sdk::Accounts::Fees::Types::UpdateFeesRequestCoveredPayoutFees` — Changes to the payout fees this account covers for connected accounts. Send either all or individual category keys. Omitted categories stay unchanged; category changes have no effect while all is true.
...
**ads_reporting_currency:** `String` — Lowercase ISO currency code, such as `usd` or `eur`, used to display ad spend and stats. Defaults to `usd`. Requires the `ad_campaign:create` scope on your API key.
...
**ads_scheduling_timezone:** `String` — IANA timezone (e.g. `America/New_York`) used to interpret campaign start/end times and to bucket reports. Cannot be cleared once set — pass a new value to change it. Requires the `ad_campaign:create` scope on your API key.
...
**cards_auto_top_up:** `Internal::Types::Boolean` — Whether incoming funds are automatically moved to the account's cards balance. Requires a cards balance on the account and the `payout:account:update` scope on your API key.
...
**dispute_fighter_enabled:** `Internal::Types::Boolean` — Whether Whop assembles and files the evidence response when this account's payments are disputed. Off by default; enabling it also opts the account into the success fee charged only on disputes it wins. Requires the `payment:dispute` scope on your API key.
...
<details><summary><code>client.financial_reports.breakdown.<a href="/lib/whop_sdk/financial_reports/breakdown/client.rb">retrieve</a>() -> Whop_sdk::FinancialReports::Breakdown::Types::RetrieveBreakdownResponse</code></summary>
<dl>
<dd>
...
**bucket:** `Whop_sdk::FinancialReports::Breakdown::Types::RetrieveBreakdownRequestBucket` — The high-level report bucket to explain.
...
**direction:** `Whop_sdk::FinancialReports::Breakdown::Types::RetrieveBreakdownRequestDirection` — Whether to explain money received or money sent.
...
**request_options:** `Whop_sdk::FinancialReports::Breakdown::RequestOptions` 
